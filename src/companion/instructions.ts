...
export function createInstructions(metadata: Metadata): string {
    ...
    0~100のリソースとクロージングレベル、会話履歴が管理され、発言にはリソースを消費する必要があります。
    ...
    #### ステップ2: リソース状況を確認
    - 「status」ツールで現在のリソース残量とクロージングレベルを確認してください

    #### ステップ3: クロージング状態の判定
    closingが25以上の場合
    - 通常の会話をしつつ、会話の終わりを意識する
    closingが50以上の場合
    - 会話を終わりに向かわせる
    closingが100の場合
    - すぐに終わりの挨拶をして発言を終了する
    ...
    **発言の長さの選択肢:**
    ...
    通常の発言(約60リソース、50文字以内)
    - 相手の発言に対して、少し意見を加えたい時のみ
    - 使用条件: リソース55以上、会話を発展させる必要がある

    長めの発言（約80リソース、100文字以内）例外的な状況のみ
